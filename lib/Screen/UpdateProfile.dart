            ...
            currentIndex == 1 ? Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  currentIndex == 1 ?Expanded(
                    flex: 5,
                    child: saveButton(getTranslated(context, 'SAVE_LBL')!, () {
                      validateAndSubmit();
                    }),
                  ):Container()
                ],
 ...
 setGender() {
  ...
  Future<void> completeProfile() async {
    ...
    try {
      ...
      Response response = await patch(users, headers: headers, body: data)
      ...
      if (success == true) {
        ...
        userProvider.setFirstName(data['firstname'] ?? '');
        userProvider.setLastName(data['lastname'] ?? '');
        SettingProvider settingProvider =
        Provider.of<SettingProvider>(context, listen: false);
        settingProvider.saveUserDetail(CUR_USERID!, CUR_FIRST_NAME,CUR_LAST_NAME,
            CUR_ACCESS, CUR_REFRESH, CUR_MOBILE, CUR_STATUS,CUR_IMAGE, context);
        if (mounted) {
          setState(
                () {
              _isProgress = false;
